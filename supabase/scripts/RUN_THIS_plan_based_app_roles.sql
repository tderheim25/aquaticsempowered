-- Run in Supabase SQL Editor if migration 0043 has not been applied.
-- Adds plan-branded app roles, founder user columns, and migrates org_admin app roles.
...
with legacy as (
  select id from public.app_roles where slug in ('org_admin', 'org_admin_legacy')
),
owner_plan as (
  select
    u.id as user_id,
...
where ar.slug = 'default_user'
  ...
  and (u.app_role_id is null or u.app_role_id in (select id from public.app_roles where slug in ('staff', 'org_admin', 'org_admin_legacy')));
