...
import { ALL_VIEW_KEYS, requireViewAccess } from "@/lib/auth/viewPermissions";
...
export async function updateRoleViewPermissionsAction(formData: FormData) {
  ...
  if (!roleId) {
    redirect("/app/admin?section=permissions&status=invalid");
  }
  ...
  if (error) {
    redirect("/app/admin?section=permissions&status=error");
  }

  revalidatePath("/app");
  revalidatePath("/app/admin");
  revalidatePath("/app/admin/permissions");
  redirect(`/app/admin?section=permissions&status=updated&roleId=${encodeURIComponent(roleId)}`);
}
...
export async function createAppRoleAction(formData: FormData) {
  ...
  if (baseRole?.id) {
    ...
    if (copyRows && copyRows.length > 0) {
      ...
      await admin.from("app_role_view_permissions").insert(payload);
    }
  } else {
    const payload = ALL_VIEW_KEYS.map((viewKey) => ({
      role_id: inserted.id,
  ...
  redirect(`/app/admin?section=permissions&status=role_created&label=${encodeURIComponent(label)}`);
}
...
export async function updateAppRoleAction(formData: FormData) {
  ...
  if (existing.is_builtin) {
    redirect("/app/admin?section=permissions&status=builtin_locked");
  }
  ...
  revalidatePath("/app/admin/permissions");
  revalidatePath("/app/admin/users");
  redirect("/app/admin?section=permissions&status=role_updated");
}
...
export async function deleteAppRoleAction(formData: FormData) {
  ...
  if ((assignedUsers ?? 0) > 0) {
    redirect("/app/admin?section=permissions&status=role_in_use");
  }
  ...
  revalidatePath("/app/admin/permissions");
  revalidatePath("/app/admin/users");
  redirect("/app/admin?section=permissions&status=role_deleted");
}
